...
async def reload_settings_from_db():
    ...
    try:
        ...
        logger.info(
            f"DB ayarlari yuklendi: ZScore={ZSCORE_ENABLED}, IF={IF_ENABLED}, "
            f"p99_disaster={rule_engine_module.ELAPSED_P99_DISASTER}, "
            f"p95_high={rule_engine_module.ELAPSED_P95_HIGH}, "
            f"min_slow_tx={rule_engine_module.MIN_SLOW_TX_COUNT}"
        )
    except Exception as e:
...
async def processing_loop():
    ...
    while True:
        ...
        try:
            ...
            if last_reload:
                elapsed_secs = (datetime.utcnow() - datetime.fromisoformat(last_reload)).total_seconds()
                if elapsed_secs >= 60:
                    await reload_settings_from_db()
            else:
                await reload_settings_from_db()
            ...
            if _stats["total_batches"] % 10 == 0:
                await save_detection_run(
                    started_at=started_at,
                    logs_processed=batch_logs,
...
def calculate_features(service, channel, logs):
    ...
    p95_idx = int(n * 0.95)
    p99_idx = int(n * 0.99)
    elapsed_p95 = elapsed_sorted[min(p95_idx, n-1)] if elapsed_sorted else 0
    elapsed_p99 = elapsed_sorted[min(p99_idx, n-1)] if elapsed_sorted else 0

    # Kac islem eslegi asti
    ...
    return {
        ...
        "slow_count_disaster": slow_count_disaster,
        "slow_count_high": slow_count_high,
    ...
    return {
        **_stats,
        ...
        "rule_engine_settings": {
            ...
            "sliding_window_seconds": rule_engine_module.SLIDING_WINDOW_SECONDS,
        }
    }
...
async def trigger_reload_settings():
    await reload_settings_from_db()
    return {
        "status": "ok",
        "zscore_enabled": ZSCORE_ENABLED,
