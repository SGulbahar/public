...
from engine.notification import notification_service
...
async def reload_settings_from_db():
    ...
    try:
        ...
        logger.info(
            f"Ayarlar yuklendi: ZScore={ZSCORE_ENABLED}, IF={IF_ENABLED}, "
            f"p99={rule_engine_module.ELAPSED_P99_DISASTER}, p95={rule_engine_module.ELAPSED_P95_HIGH}, "
            f"bildirim={notification_service.enabled}"
        )
    except Exception as e:
...
async def test_notification():
    """Bildirim testi icin ornek anomali gonder."""
    test_anomali = {
        "service": "TEST_SERVICE",
        "channel_code": "000",
        "anomaly_type": "test",
        "severity": notification_service.min_severity,
        "score": 99.9,
        "error_rate": 0.75,
        "elapsed_mean": 5000,
        "tx_count": 100,
    ...
    # Cooldown'u bypass et
    from engine.notification import _notification_cooldown
    _notification_cooldown.clear()
    await notification_service.anomali_bildir(test_anomali)
    return {"status": "ok", "message": "Test bildirimi gonderildi"}
