...
async def reload_settings_from_db():
    ...
    try:
        ...
        logger.info(f"DB ayarlari yuklendi: ZScore={ZSCORE_ENABLED}, IF={IF_ENABLED}, elapsed_disaster={engine_settings.rule_elapsed_disaster}")
    except Exception as e:
...
async def processing_loop():
    ...
    while True:
        ...
        batch_anomalies = 0
        try:
            # Her 60 batch'te bir ayarlari DB'den yenile
            if _stats["total_batches"] > 0 and _stats["total_batches"] % 60 == 0:
                await reload_settings_from_db()
...
async def trigger_reload_settings():
    await reload_settings_from_db()
    return {"status": "ok", "zscore_enabled": ZSCORE_ENABLED, "if_enabled": IF_ENABLED}
