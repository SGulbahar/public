...
async def veri_cek(dsn: str) -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.DataFrame([{k: to_float(v) for k, v in dict(r).items()} for r in rows])
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Tum sayisal sutunlari float'a cevir
    for col in ['mean_error','std_error','mean_elapsed','std_elapsed','sample_count','hour_bucket','weekday']:
...
def veri_hazirla(df_normal: pd.DataFrame, df_anomali: pd.DataFrame) -> tuple:
    ...
    # Kategorik sutunlari string yap - CatBoost integer/string bekliyor
