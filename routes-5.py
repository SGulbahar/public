...
async def kullanici_listesi(request: Request, db: AsyncSession = Depends(get_session)):
    result = await db.execute(text("""
        SELECT u.id, u.username, u.email, u.is_active, u.auth_source,
...
async def kullanici_ekle(
    ...
    if not username or not password:
        raise HTTPException(status_code=400, detail="Kullanici adi ve sifre zorunludur")
    if len(password) < 8:
    ...
    try:
        ...
        await db.commit()
        await write_audit(db, user.get("sub", "unknown"), "user.create", username, f"Yeni kullanici olusturuldu", request)
...
async def kullanici_guncelle(
    ...
    await db.commit()
    await write_audit(db, user.get("sub", "unknown"), "user.update", str(user_id), "Kullanici guncellendi", request)
    return {"status": "ok"}
...
async def kullanici_sil(
    ...
    # Kendini silemesin
    mevcut = await db.execute(text("SELECT username FROM users WHERE id = :id"), {"id": user_id})
    row = mevcut.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Kullanici bulunamadi")
    if row.username == user.get("sub"):
        raise HTTPException(status_code=400, detail="Kendi hesabinizi silemezsiniz")

    await db.execute(text("DELETE FROM user_roles WHERE user_id = :id"), {"id": user_id})
    await db.execute(text("DELETE FROM users WHERE id = :id"), {"id": user_id})
    await db.commit()
    await write_audit(db, user.get("sub", "unknown"), "user.delete", row.username, "Kullanici silindi", request)
    return {"status": "ok"}
