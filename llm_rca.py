...
class LLMRCAService:
    ...
    def _prompt_olustur(self, anomali: dict) -> str:
        ...
        prompt = f"""Bankacılık sisteminde anomali tespit edildi. Kısa ve net kök neden analizi yap.
...
- Hata Oranı: %{(anomali.get('error_rate', 0) * 100):.1f}
- Ort. Yanıt Süresi: {anomali.get('elapsed_mean', 0):.0f} ms
- İşlem Sayısı: {anomali.get('tx_count', 0)}
{f'- Result Dağılımı: {rd_str}' if rd_str else ''}

Lütfen şunları belirt:
1. Olası kök neden (1-2 cümle)
2. Önerilen aksiyon (1-2 madde)

Yanıtı Türkçe ver, teknik ve öz tut."""

        return prompt

    async def rca_analiz(self, anomali: dict) -> Optional[str]:
        ...
        try:
            prompt = self._prompt_olustur(anomali)
