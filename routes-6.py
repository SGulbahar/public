...
async def mark_false_positive(
    ...
    # FP isaretlenenler acknowledge edilmis sayilir
    if not anomaly.acknowledged:
        anomaly.acknowledged = True
        anomaly.acknowledged_by = user.get("sub")
        anomaly.acknowledged_at = datetime.utcnow()
    await db.commit()
    logger.info(f"FP isaretlendi: anomaly_id={anomaly_id} by={user.get('sub')} reason={body.reason}")
    return {"message": "False positive olarak isaretlendi", "anomaly_id": anomaly_id}


@router.get("/anomalies/stats/false-positive")
async def false_positive_stats(db: AsyncSession = Depends(get_session)):
    ...
    return [{"anomaly_type": r.anomaly_type, "total": r.total, "fp_count": r.fp_count, "fp_rate": float(r.fp_rate or 0)} for r in rows]
...
async def dashboard_stats(db: AsyncSession = Depends(get_session)):
    ...
    fp_count = (await db.execute(
        select(func.count(AnomalyEvent.id))
        .where(AnomalyEvent.is_false_positive == True)
    )).scalar()
...
async def get_settings(db: AsyncSession = Depends(get_session)):
    result = await db.execute(
        text("SELECT key, value, description, updated_at, updated_by FROM system_settings ORDER BY key")
    )
    rows = result.fetchall()
    return [
        {
