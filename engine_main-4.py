...
async def _llm_rca_gonder(anomali_dict: dict):
    ...
    try:
        ai_analysis = await llm_rca_service.rca_analiz(anomali_dict)
        if ai_analysis:
            anomali_dict["ai_analysis"] = ai_analysis
            # DB'ye direkt UPDATE at
            try:
                conn = await get_db_conn()
                await conn.execute(
                ...
                await conn.close()
                logger.info(f"LLM RCA DB'ye yazildi: {anomali_dict.get('service')}")
            except Exception as db_e:
                logger.error(f"LLM RCA DB yazma hatasi: {db_e}")
    except Exception as e:
