...
from engine.zabbix_collector import collector_baslat, collector_al
from engine.service_health import sh_baslat, sh_al
...
async def lifespan(app: FastAPI):
    ...
    sh = sh_baslat(engine_settings.database_url.replace("postgresql+asyncpg://","postgresql://"))
    await sh.baglanti_ac()
    sh.baslat()
    logger.info('ServiceHealth Manager hazir')
    collector = collector_baslat(engine_settings.database_url.replace("postgresql+asyncpg://","postgresql://"))
    await collector.baglanti_ac()
    collector.baslat()
    logger.info('Zabbix Event Collector hazir')
...
async def process_batch(logs):
    ...
    for key, group_logs in groups.items():
        ...
        for f in finals:
            ...
            # Service Health guncelle
            try:
                sh_mgr = sh_al()
                if sh_mgr:
                    asyncio.create_task(sh_mgr.anomali_isle(anomali_dict))
            except Exception as sh_e:
                logger.error(f"ServiceHealth hatasi: {sh_e}")
            # Zabbix gonder
            try:
                zabbix = zabbix_al()
                if zabbix:
...
@app.post("/rca-trigger")
async def rca_trigger(payload: dict):
    """
    Harici kaynaklardan (ML Inference, ratdan gelen anomaliler) RCA tetiklemek
    icin kullanilir. payload bir anomali dict'i olmalidir (id, service,
    channel_code, severity, anomaly_type, summary, error_rate, elapsed_mean,
    tx_count alanlarini icermeli).
    """
    try:
        anomali_dict = dict(payload)
        anomali_dict.setdefault("anomaly_type", "external")
        asyncio.create_task(_llm_rca_gonder(anomali_dict))
        try:
            zabbix = zabbix_al()
            if zabbix:
            ...
            logger.error(f"Zabbix gonderme hatasi (rca-trigger): {zb_e}")
        try:
            cascade = cascade_detector_al()
            if cascade:
                asyncio.create_task(cascade.kontrol_et([anomali_dict]))
        except Exception as cas_e:
            logger.error(f"Cascade kontrol hatasi (rca-trigger): {cas_e}")
        return {"status": "ok", "service": anomali_dict.get("service")}
    except Exception as e:
        logger.error(f"rca-trigger hatasi: {e}")
        return {"status": "error", "message": str(e)}
