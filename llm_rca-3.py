...
{rag_context}"""
        ...
        # RAG'dan sadece tekrarlayan bilgisini al
        tekrarlayan_bilgi = ""
        if rag_bolum:
            # Sadece gecmis anomali sayisi varsa ekle
            lines = rag_bolum.split("\n")
            for line in lines:
                if "Toplam anomali" in line or "once anomali uretmemis" in line:
                    tekrarlayan_bilgi = line.strip()
                    break

        prompt = f"""Bankacılık sisteminde anomali tespit edildi. Operasyon ekibine yönelik teknik analiz yap.

=== ANOMALİ BİLGİLERİ ===
Kanal: {anomali.get('channel_code', '-')} | Servis: {anomali.get('service', '-')}
Şiddet: {severity_tr} | Kategori: {category_tr}
Özet: {anomali.get('summary', '-')}
Hata Oranı: %{(anomali.get('error_rate', 0) * 100):.1f} | Yanıt Süresi: {anomali.get('elapsed_mean', 0):.0f}ms | İşlem Sayısı: {anomali.get('tx_count', 0)}
{f'Hata Dağılımı: {rd_str}' if rd_str else ''}
{f'Geçmiş: {tekrarlayan_bilgi}' if tekrarlayan_bilgi else ''}

=== GÖREV ===
Aşağıdaki formatta yanıt ver:

**Hata Açıklaması:** Hata kodunun teknik anlamını 1 cümleyle açıkla (DB ile ilişkilendirme yapma, hata kodunun genel teknik anlamına odaklan).

**Olası Nedenler:** Bu tip hatanın 2-3 genel nedeni (timeout ise: DB yoğunluğu, CPU/memory artışı, harici servis gecikmesi gibi — somut ve genel geçerli nedenler).

**Kontrol Edilmesi Gerekenler:** Hangi bağımlılıklar ve sistemler kontrol edilmeli (1-2 madde).

**Durum:** Tekrarlayan mı yoksa yeni mi (geçmiş veriye göre 1 cümle).

NOT: Servis adındaki kısaltmaları (MMS, DGB, URN vb.) veritabanı tablosu olarak yorumlama. Yanıt Türkçe ve kısa olsun."""
