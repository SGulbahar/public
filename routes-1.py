...
async def ingest_anomalies(
    ...
    try:
        ...
        logger.info(f"Ingest body ilk 200 karakter: {body[:200]}")
        ...
        for a in anomalies:
            ...
            event = AnomalyEvent(
                ...
                result_code=int(rc),
                result_desc=error_code_service.describe(int(rc)),
                result_category=error_code_service.category(int(rc)),
            ...
            db.add(event)
            await db.flush()
            logger.info(f"Anomali flush edildi: id={event.id}")
            ...
            try:
                ...
                logger.error(f"Zabbix hatasi (devam ediliyor): {ze}")

            saved += 1

        await db.commit()
        logger.info(f"{saved} anomali DB'ye kaydedildi")
        return {"saved": saved}
