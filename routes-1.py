...
async def katalog_servis_kaydet(
    ...
    await db.commit()
    await audit_log(db, current_user.username, "catalog.service.upsert", service_name,
...
async def katalog_servis_sil(
    ...
    await db.execute(text("DELETE FROM service_catalog WHERE service_name = :sn"),
                     {"sn": service_name})
    await db.commit()
    await audit_log(db, current_user.username, "catalog.service.delete", service_name,
...
async def tag_listesi(db: AsyncSession = Depends(get_db)):
