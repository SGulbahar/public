...
async def write_audit(
    ...
    try:
        ...
        logger.error(f"Audit log yazma hatasi: {e}")
...
async def acknowledge_anomaly(
    ...
    await write_audit(
        db, user.get("sub"), "anomaly.acknowledge",
...
async def mark_false_positive(
    ...
    if not anomaly.acknowledged:
        ...
        anomaly.acknowledged_at = datetime.utcnow()
    await write_audit(
        db, user.get("sub"), "anomaly.false_positive",
        f"anomaly_events/{anomaly_id}",
        f"service={anomaly.service} channel={anomaly.channel_code} reason={body.reason}",
        ip_address=get_client_ip(request)
    )
    await db.commit()
...
async def list_runs(
    ...
    rows = (await db.execute(
        select(DetectionRun).order_by(desc(DetectionRun.id)).limit(limit)
    )).scalars().all()
    return [
        {
            "id": r.id,
...
async def reload_error_codes(
    ...
    await write_audit(
        db, user.get("sub"), "error_codes.reload",
        "error_codes",
        f"reloaded={count}",
        ip_address=get_client_ip(request)
    )
...
async def upsert_channel_mapping(
    ...
    await write_audit(
        db, user.get("sub"), "channel_mapping.upsert",
        f"channel_mappings/{body.channel_code}",
        f"channel_name={body.channel_name}",
        ip_address=get_client_ip(request)
    )
    await db.commit()
...
async def delete_channel_mapping(
    ...
    await write_audit(
        db, user.get("sub"), "channel_mapping.delete",
        f"channel_mappings/{channel_code}",
        ip_address=get_client_ip(request)
    )
    await db.commit()
...
async def approve_action(
    ...
    await write_audit(
        db, user.get("sub"), "sre_action.approve",
        f"sre_actions/{action_id}",
...
async def reject_action(
    ...
    action.approved_at = datetime.utcnow()
    await write_audit(
        db, user.get("sub"), "sre_action.reject",
        f"sre_actions/{action_id}",
        f"action_type={action.action_type} target={action.target}",
        ip_address=get_client_ip(request)
    )
