...
@router.get("/zabbix/feed")
async def zabbix_feed(db: AsyncSession = Depends(get_session)):
    """
    Zabbix HTTP Agent master item icin JSON feed.
    Aktif anomalileri JSON olarak dondurur.
    Kimlik dogrulama gerektirmez - Zabbix polling icin.
    """
    result = await db.execute(text("""
        SELECT
            service, channel_code, severity, anomaly_type,
            summary, error_rate, elapsed_mean, detected_at,
            acknowledged, is_false_positive
        FROM anomaly_events
    ...
    anomaliler = [r for r in rows if not r.acknowledged]

    disaster = [a for a in anomaliler if a.severity == 'DISASTER']
    high = [a for a in anomaliler if a.severity == 'HIGH']
    warning = [a for a in anomaliler if a.severity == 'WARNING']

    return {
        "timestamp": datetime.utcnow().isoformat(),
        "active_count": len(anomaliler),
        "disaster_count": len(disaster),
        "high_count": len(high),
        "warning_count": len(warning),
        "status": "DISASTER" if disaster else "HIGH" if high else "WARNING" if warning else "OK",
        "anomalies": [{
            "service": a.service,
            "channel": a.channel_code,
            "severity": a.severity,
            "type": a.anomaly_type,
            "summary": (a.summary or "")[:100],
            "error_rate_pct": round((a.error_rate or 0) * 100, 1),
            "elapsed_ms": int(a.elapsed_mean or 0),
            "detected_at": a.detected_at.strftime("%Y-%m-%d %H:%M:%S") if a.detected_at else ""
        } for a in anomaliler[:20]]
    }
