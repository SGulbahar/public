...
- p95/p99 elapsed (anlık spike false positive engeli)
"""
import logging
import time
...
class RuleEngine:
    ...
    def detect(self, features: dict) -> list:
        ...
        if not self._in_cooldown(key):
            ...
            if elapsed_p99 >= ELAPSED_P99_DISASTER:
                anomalies.append(RuleAnomaly(
                    ...
                    result_code=0,
                    summary=f"{service} [{channel}] - Kritik gecikme: p99={int(elapsed_p99)}ms",
                    details=f"Islemlerin %99u {int(elapsed_p99)}ms uzun surdu. Esik: {ELAPSED_P99_DISASTER}ms. Islem: {tx_count}",
                ))

            # Kural 2: p95 elapsed HIGH
            elif elapsed_p95 >= ELAPSED_P95_HIGH:
                anomalies.append(RuleAnomaly(
                    ...
                    error_rate=error_rate,
                    result_code=0,
                    summary=f"{service} [{channel}] - Yuksek gecikme: p95={int(elapsed_p95)}ms",
                    details=f"Islemlerin %95i {int(elapsed_p95)}ms uzun surdu. Esik: {ELAPSED_P95_HIGH}ms. Islem: {tx_count}",
                ))
