"""
REST API endpoint'leri - Kapsamli audit log ile
"""
import logging
import traceback
...
async def write_audit(db, user, action, target, detail="", result="success", ip_address=""):
    try:
        ...
        """), {"user": user, "action": action, "target": target, "detail": detail, "ip": ip_address, "result": result})
...
async def acknowledge_anomaly(
    ...
    await write_audit(db, user.get("sub"), "anomaly.acknowledge", f"anomaly_events/{anomaly_id}",
...
async def mark_false_positive(
    ...
    if not anomaly.acknowledged:
        ...
        anomaly.acknowledged_by = user.get("sub")
        anomaly.acknowledged_at = datetime.utcnow()
    await write_audit(db, user.get("sub"), "anomaly.false_positive", f"anomaly_events/{anomaly_id}",
        f"service={anomaly.service} reason={body.reason}", ip_address=get_client_ip(request))
    await db.commit()
...
async def reload_error_codes(request: Request, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    count = await error_code_service.reload()
    await write_audit(db, user.get("sub"), "error_codes.reload", "error_codes", f"reloaded={count}", ip_address=get_client_ip(request))
    await db.commit()
...
async def list_channel_mappings(db: AsyncSession = Depends(get_session)):
    ...
    return [{"channel_code": r.channel_code, "channel_name": r.channel_name, "description": r.description,
             "updated_at": r.updated_at.strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z" if r.updated_at else None} for r in rows]
...
async def upsert_channel_mapping(
    ...
    await write_audit(db, user.get("sub"), "channel_mapping.upsert", f"channel_mappings/{body.channel_code}",
...
async def delete_channel_mapping(
    ...
    await write_audit(db, user.get("sub"), "channel_mapping.delete", f"channel_mappings/{channel_code}", ip_address=get_client_ip(request))
    await db.commit()
...
async def approve_action(
    ...
    await write_audit(db, user.get("sub"), "sre_action.approve", f"sre_actions/{action_id}",
        f"action_type={action.action_type}", ip_address=get_client_ip(request))
    await db.commit()
...
async def reject_action(
    ...
    action.approved_by = user.get("sub")
    action.approved_at = datetime.utcnow()
    await write_audit(db, user.get("sub"), "sre_action.reject", f"sre_actions/{action_id}",
        f"action_type={action.action_type}", ip_address=get_client_ip(request))
    await db.commit()
...
async def dashboard_stats(db: AsyncSession = Depends(get_session)):
    ...
    # Servis bazli dagilim - son 24 saat
    ...
    """))
    by_service = [{"service": r.service, "count": int(r.cnt), "disaster": int(r.disaster), "high": int(r.high), "warning": int(r.warning)} for r in svc_result.fetchall()]

    # Kanal bazli dagilim - son 24 saat
    ch_result = await db.execute(text("""
        SELECT a.channel_code,
               COALESCE(c.channel_name, a.channel_code) as channel_name,
               COUNT(*) as cnt
    ...
    """))
    by_channel = [{"channel_code": r.channel_code, "channel_name": r.channel_name, "count": int(r.cnt)} for r in ch_result.fetchall()]

    # Son 24 saat saatlik trend
    ...
    """))
    trend = [{"hour": r.hour.strftime("%H:00"), "count": int(r.cnt)} for r in trend_result.fetchall()]
...
async def update_settings(
    ...
    try:
        ...
        for key, value in body.items():
            result = await db.execute(text("SELECT key, value FROM system_settings WHERE key = :key"), {"key": key})
            row = result.fetchone()
            if row:
                old_value = row.value
                await db.execute(
        ...
        await write_audit(db, username, "settings.update", "system_settings",
            "; ".join(changes) if changes else "degisiklik yok", ip_address=get_client_ip(request))
        await db.commit()
        try:
...
async def list_audit_logs(
    ...
    return [{"id": r.id,
             ...
             "detail": r.detail, "ip_address": r.ip_address, "result": r.result} for r in rows]
