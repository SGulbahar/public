...
- Deduplication
- Hata kodu sliding window (30 dakika)
"""
import logging
import time
...
SLIDING_WINDOW_SECONDS = 1800  # 30 dakika
...
class RuleEngine:
    def __init__(self):
        ...
        # key: "service:channel:rc{code}" → [(timestamp, count), ...]
    ...
    def detect(self, features: dict) -> list:
        ...
        if tx_count < MIN_TX_COUNT:
            # Hata kodu penceresini guncelle (az tx olsa bile)
            self._update_error_code_window(service, channel, result_codes)
            return []

        anomalies = []

        # Elapsed ve error rate kurallari (cooldown ile)
        ...
        # Kural 5: Hata Kodu Sliding Window (ayri cooldown)
        self._update_error_code_window(service, channel, result_codes)
    ...
    def _update_error_code_window(self, service, channel, result_codes):
        """Her hata kodunu sliding window'a ekler."""
        now = time.time()
    ...
    def _check_error_code_window(self, service, channel, elapsed_mean) -> list:
        """Son 30 dakikadaki hata kodu toplamlarini kontrol eder."""
        anomalies = []
        now = time.time()

        for ec_key, entries in list(self._error_code_window.items()):
            if not ec_key.startswith(f"{service}:{channel}:rc"):
                continue
            ...
            if rc < 7500 and window_count >= SYS_ERROR_CODE_THRESHOLD:
                ...
                anomalies.append(RuleAnomaly(
                    ...
                    error_rate=0,
                    result_code=rc,
                    summary=f"{service} [{channel}] - SYS Hata Kodu Spike: {rc} (30dk'da {window_count} kez)",
                ...
                anomalies.append(RuleAnomaly(
                    ...
                    error_rate=0,
                    result_code=rc,
                    summary=f"{service} [{channel}] - BIZ Hata Kodu Spike: {rc} (30dk'da {window_count} kez)",
    ...
    def cooldown_stats(self) -> dict:
        ...
        window_keys = len(self._error_code_window)
        return {
            "active_cooldowns": active,
            "active_error_code_cooldowns": ec_active,
            "window_tracked_codes": window_keys,
        }
